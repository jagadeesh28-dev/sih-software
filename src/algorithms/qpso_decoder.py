"""
A2: QPSO + Decoder / Repair Mechanism.
Isolates the effect of representation and deterministic repair:
- Continuous QPSO updates
- Candidate vector is repaired prior to evaluation:
  * Eliminates demand assignment collisions
  * Replaces incompatible fuels
  * Validates cargo vs. deadweight
- Uses STANDARD scalar penalized fitness comparison (NO Deb rules!)
- Tracks repair count, repair rate, and diversity impact
"""
...
class QPSODecoderOptimizer(BaseFleetOptimizer):
    """A2: QPSO with Decoder/Repair Mechanism and Standard Fitness Selection."""
    ...
    def __init__(
        ...
        super().__init__(name="A2_QPSO_Decoder_Repair", seed=seed)
    ...
    def optimize(
        ...
        # 1. Initialize swarm and apply repair
        ...
        for i in range(self.n_particles):
            if evaluator.evaluation_count >= eval_cap:
                break
            # Apply repair before evaluation
            repaired_x, was_repaired, _ = self.repairer.repair_vector(X[i])
            X[i] = repaired_x.copy()
        ...
        # 2. Main generation loop with repair and standard selection
        t = 0
        ...
        return OptimizationResult(
            algorithm="A2_QPSO_Decoder_Repair",
            seed=self.seed,
            run_id=f"A2_seed_{self.seed}",
