"""
SIH26138 — Egreen Quanta: Eleven Production Demonstration Scenes (backend-computed; Scene 7 cites a frozen artifact).
...
# Verified scene inputs, shared with the HTTP API (api/main.py).
SCENE1_INPUT = {
...
def print_jury_card(res, title="JURY VERIFICATION BLOCK"):
    ...
    print(f"  --- {title} ---")
    if res.get("fuel_prediction") is None:
        print("  Prediction:  NONE (request rejected)")
    else:
    ...
    if unc:
        print(f"  Uncertainty: [{unc['lower_bound_kg_h']:.2f}, {unc['upper_bound_kg_h']:.2f}] kg/h")


SCENE_CHECKS = []

# Scene 7 live optimizer run (post-fix; Hybrid QI A5 returns a penalty-free plan at this seed/budget).
OPT_SEED = 1005
OPT_BUDGET = 2500


def check(scene: int, ok: bool, what: str):
    """Record a backend-derived sanity check for a scene; failures set a non-zero exit code."""
    SCENE_CHECKS.append((scene, bool(ok), what))
    if not ok:
        print(f"  !! CHECK FAILED (scene {scene}): {what}")
...
def run_all_demo_scenes():
    ...
    print("-" * 65)
    scene1_input = copy.deepcopy(SCENE1_INPUT)
    ...
    print(f"  Interval Width:      {res1['uncertainty']['interval_width_kg_h']:.2f} kg/h (split-conformal, 90% nominal)")
    ...
    check(1, (res1.get("fuel_prediction") or 0) > 0 and res1["routing_status"] != "REJECT", "normal cruise yields a served prediction")
    ...
    check(2, delta_fuel > 0, "higher speed and sea state increase predicted fuel")
    ...
    check(3, saving_pct > 0, "slow steaming reduces predicted fuel")
    ...
    vlsfo_rate = res1["fuel_prediction"]
    print("  Physics Basis: equal shaft energy; m_fuel = m_VLSFO * LHV_VLSFO / LHV_fuel (backend SIHObjectiveEngine)")
    print(f"  VLSFO reference rate:      {vlsfo_rate:.2f} kg/h (Scene 1 model prediction)")
    print(f"  {'Fuel':<17} | {'Rate (kg/h)':<11} | {'TtW (t/h)':<10} | {'WtW (t/h)':<10} | {'Basis'}")
    print("  " + "-" * 78)
    alt = {}
    for f_code, f_name in [("vlsfo", "VLSFO"), ("mgo", "MGO"), ("fossil_lng", "Fossil LNG"),
                           ("bio_methanol", "Bio-Methanol"), ("green_ammonia", "Green Ammonia"),
                           ("liquid_hydrogen", "Liquid Hydrogen")]:
        # One-hour leg at 14.5 kn so tonnes per leg == tonnes per hour.
        ev = sih_engine.evaluate_voyage(
            vessel_id="CPS_Poseidon", vessel_type="passenger_cruise", speed_knots=14.5,
            voyage_distance_nm=14.5, schedule_deadline_hours=1.0,
            baseline_fuel_rate_kg_h=vlsfo_rate, fuel_type=f_code,
        )
        alt[f_code] = ev
        basis = "MODEL PREDICTION" if f_code == "vlsfo" else "SCENARIO ESTIMATE"
        print(f"  {f_name:<17} | {ev.fuel_tonnes * 1000:<11.2f} | {ev.ttw_ghg_tonnes:<10.3f} | {ev.lifecycle_ghg_tonnes:<10.3f} | {basis}")
    print("  >> NOTE: No alternative-fuel telemetry exists; non-VLSFO rows are energy-equivalent scenario estimates.")
    check(4, all(alt[f].fuel_type == f for f in alt), "every requested fuel resolved to its own pathway (no silent VLSFO)")
    check(4, alt["bio_methanol"].fuel_tonnes > alt["vlsfo"].fuel_tonnes > alt["liquid_hydrogen"].fuel_tonnes,
          "fuel mass scales inversely with LHV")
    ...
    print("-" * 65)
    ood_input = copy.deepcopy(OOD_STORM_INPUT)
    ...
    print(f"  Envelope Distance:   {res_ood['envelope_distance']:.3f} (warning > 1.00, OOD > 1.50, reject > 3.00)")
    ...
    check(5, res_ood["routing_status"] in ("REJECT", "EMERGENCY_PHYSICS") and not res_ood["in_domain"],
          "storm state is classified out-of-distribution (no ML recommendation)")
    ...
    check(6, res_fail["prediction_source"] == "MODEL_REAL_04", "QI booster failure falls back to MODEL-REAL-04")
    ...
    fleet_ev = Phase4FleetEvaluator(surrogates=load_real_surrogates())
    fleet_ev.weights = np.array([0.35, 0.35, 0.30, 0.0, 0.0])
    xl, xu = fleet_ev.get_bounds()
    res = A5CompleteHybridQIOptimizer(seed=OPT_SEED, n_particles=50, max_iterations=50).optimize(
        CommonFleetEvaluator(fleet_ev, max_budget=OPT_BUDGET), xl, xu, OPT_BUDGET)
    o = res.best_output
    print(f"  Source:              LIVE post-fix run — Hybrid QI (A5), seed {OPT_SEED}, {OPT_BUDGET} evaluations, formulation D")
    print(f"  Plan:                fuel {o.fuel_tonnes:.2f} t | cost ${o.opex_usd:,.2f} | WtW {o.ghg_tonnes:.2f} tCO2e | delay {o.delay_hours:.2f} h | penalty {o.penalty:.0f}")
    for vid, sp in o.speed_decisions.items():
        print(f"    {vid:<13} {o.assigned_demands[vid]} @ {sp:.2f} kn on {o.fuel_decisions[vid]}")
    print("  Phase 5 frozen benchmark (825k evals): PRE-FIX HISTORICAL — not used as current evidence.")
    print("  Operator Role:       HUMAN-IN-THE-LOOP decision support (advisory recommendations only)")
    check(7, bool(o.is_feasible and o.penalty <= 0.0), "live optimizer plan is hard-feasible and penalty-free")
    check(7, all(v.min_speed_knots <= o.speed_decisions[v.vessel_id] <= v.max_speed_knots for v in fleet_ev.vessels),
          "every commanded speed lies inside its vessel band")
    ...
    print("-" * 65)
    vessels_data = copy.deepcopy(VESSEL_TYPE_SCENE)
    ...
    for v_id, v_type, disp, draft in vessels_data:
        ...
        check(8, (res_v.get("fuel_prediction") or 0) > 0, f"{v_id} served with its own vessel_type")
    print("  >> vessel_type is an explicit model feature. Physics baseline uses a declared proxy hull")
    print("     (container_feeder) for all three vessels; the learned residual absorbs the hull mismatch.")
    ...
    print("  >> C_total = C_fuel + C_electricity + C_OPS + C_carbon + C_schedule (baseline fuel rates are stated inputs).")
    for c in (cost_fuel_focus, cost_cost_focus):
        parts = c.fuel_cost_usd + c.shore_power_cost_usd + c.carbon_cost_usd + c.schedule_penalty_usd + c.fueleu_penalty_usd
        check(9, c.operational_cost_usd > 0 and c.operational_cost_usd >= parts - 0.05, "OPEX is positive and covers its components")
    ...
    print("\n[SCENE 10] LIFECYCLE WELL-TO-WAKE GHG MINIMIZATION (SIH Core Requirement 3)")
    print("-" * 65)
    f_scenarios = copy.deepcopy(LIFECYCLE_FUEL_SCENARIOS)
    ...
    for f_code, f_label, shore_pwr in f_scenarios:
        ...
        wtw[f_code] = ev.lifecycle_ghg_tonnes
        # Input rate of 2500 kg/h VLSFO is an assumption, so every row is a scenario estimate.
        status = "SCENARIO BASELINE" if f_code == "vlsfo" else "SCENARIO ESTIMATE"
    ...
    check(10, wtw["green_ammonia"] < wtw["vlsfo"] and wtw["liquid_hydrogen"] < wtw["vlsfo"],
          "zero-carbon pathways have lower WtW GHG than VLSFO")
    ...
    df_u = df_p.drop_duplicates(subset=obj_cols).reset_index(drop=True)
    print(f"  Source: {pareto_file.relative_to(REPO_ROOT).as_posix()} (stored optimizer output)")
    print(f"  {len(df_p)} archived rows, {len(df_u)} distinct objective vectors (Fuel, Cost, GHG, Schedule):")
    for idx, row in df_u.head(5).iterrows():
    ...
    check(11, len(df_u) >= 2, "Pareto archive contains at least two distinct trade-off points")
    check(11, bool(df_p["is_feasible"].astype(str).str.lower().eq("true").all() and (df_p["penalty"] <= 0.0).all()),
          "every stored Pareto point is hard-feasible and penalty-free")
    from src.benchmark.metrics import is_pareto_efficient
    check(11, bool(is_pareto_efficient(df_u[["fuel_tonnes", "cost_usd", "ghg_tonnes"]].values).all()),
          "no stored Pareto point is dominated by another in fuel, cost and WtW GHG")

    scenes = sorted({s for s, _, _ in SCENE_CHECKS})
    failed = sorted({s for s, ok, _ in SCENE_CHECKS if not ok})
    print("\n" + "=" * 78)
    print(f"  DEMO RESULT: {len(scenes) - len(failed)}/{len(scenes)} scenes passed backend checks"
...
if __name__ == "__main__":
    sys.exit(0 if run_all_demo_scenes() else 1)
